...
def dependency_layers(edges):
    ...
    # Non-recursive layered Kahn: each round peels off all indegree-zero
    # nodes (sorted by id == first-appearance order), then decrements the
    # indegrees of their dependents for the next round.
    ...
    while remaining:
        ...
        if not current:
            # No progress: the remaining ids form a real directed cycle or
            # a subgraph blocked by one. Surface them in first-appearance
            # order, which is the natural id order.
            blocked = [nodes_by_id[nid] for nid in remaining]
